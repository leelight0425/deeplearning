...
if __name__ == "__main__":
    ...
    # ---- 命令行参数 ----
    ap = argparse.ArgumentParser(description="Restormer 训练")
    ap.add_argument("--data-root", default="dataset", help="数据集根目录")
    ap.add_argument("--patch-size", type=int, default=128, help="训练 patch 大小")
    ap.add_argument("--sigma-min", type=float, default=10, help="噪声标准差下限")
    ap.add_argument("--sigma-max", type=float, default=50, help="噪声标准差上限")
    ap.add_argument("--batch-size", type=int, default=8)
    ap.add_argument("--epochs", type=int, default=50)
    ap.add_argument("--lr", type=float, default=2e-4)
    ap.add_argument("--dim", type=int, default=48, help="初始特征维度")
    ap.add_argument("--num-blocks", type=int, nargs=4, default=[4, 6, 6, 8],
                    help="各层 Transformer Block 数量")
    ap.add_argument("--num-heads", type=int, default=4)
    ap.add_argument("--num-workers", type=int, default=0, help="DataLoader 子进程数")
    args = ap.parse_args()
    ...
    # 数据准备 — Restormer 经典管线
    # ================================================================
    from dataset_restormer import (
        build_dataset, SyntheticDenoisingDataset, PairedImageDataset
    )

    DATA_ROOT = args.data_root
    VAL_ROOT = _os.path.join(DATA_ROOT, "val")
    ...
    # 验证集：如果 val/ 子目录存在就用，否则复用训练路径但关掉增强
    if _os.path.isdir(VAL_ROOT):
        ...
        # 无独立验证目录 → 从训练根构建一个不增强的子集
        val_set = SyntheticDenoisingDataset(
            DATA_ROOT,
            patch_size=args.patch_size,
            sigma_range=(args.sigma_min, args.sigma_max),
            augment=False,
            max_images=min(len(train_set), 800),
        )
    ...
    # 断点续训
    # ================================================================
    CHECKPOINT_PATH = "restormer_checkpoint.pt"
    start_epoch = 0
    total_step = 0
    best_psnr = 0.0
    ...
    if _os.path.exists(CHECKPOINT_PATH):
        ...
        print(f"  已恢复: epoch={start_epoch}  total_step={total_step}  "
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        remaining_min = (elapsed / epochs_done) * (args.epochs - epoch - 1) / 60
